...
export class StanzaConfigEntityManager<T> {
  ...
  private readonly unsubscribeServiceConfigListener = addServiceConfigListener(
    (state) => {
      state.initialized &&
  ...
  async shutdown(): Promise<void> {
    ...
    this.unsubscribeGuardConfigListeners.forEach((u) => {
      u();
    });
    await Promise.all(
  ...
  private getGuardEntity(context: Context): T | undefined {
    ...
    this.unsubscribeGuardConfigListeners.push(
      addGuardConfigListener(guardName, ({ config }) => {
        if (config !== undefined) {
