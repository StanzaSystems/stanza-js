    ...
    traceConfig: {
      ...
      overrides: [{
        ...
        spanSelectors: [{
          ...
          otelAttribute: 'http.server.header.yet_another_test_header_override',
    ...
    traceConfig: {
      ...
      overrides: [{
        ...
        spanSelectors: [{
          ...
          otelAttribute: 'http.server.header.second_another_test_header_override',
          value: 'another test header override value'
        }],
        sampleRate: 0.5
      }, {
        spanSelectors: [{
          ...
          otelAttribute: 'http.server.header.second_yet_another_test_header_override',
          value: 'yet-another-test header override value'
        }],
        sampleRate: 0.5
      }],
...
describe('TraceConfigOverrideAdditionalInfoPropagator', () => {
  it('should create TraceConfigOverrideAdditionalInfoPropagator', () => {
    expect(() => new TraceConfigOverrideAdditionalInfoPropagator()).not.toThrow()
  })
  ...
  describe('extract', () => {
    ...
    it('should add empty additional info if service config is not initialized', () => {
      ...
      const newContext = propagator.extract(ROOT_CONTEXT, {
    ...
    it('should return only used headers necessary for trace config overrides if service config is initialized', () => {
      getServiceConfigMock.mockImplementationOnce(() => mockServiceConfigWithOverrides)
    ...
    it('should return only used headers necessary for trace config overrides after service config is updated', () => {
      ...
      const newContext = propagator.extract(ROOT_CONTEXT, {
        ...
        'second-yet-another-test-header-override': 'second-yet-another-test-value'
      }, recordGetter)
      expect(getTraceConfigOverrideAdditionalInfo(newContext)).toEqual({
        headers: {
          'test-header-override': 'test-value',
      ...
      serviceListener({ initialized: true, data: secondMockServiceConfigWithOverrides })

      const secondContext = propagator.extract(ROOT_CONTEXT, {
        'test-header': 'test-value',
        'test-header-override': 'test-value',
        'another-test-header-override': 'another-test-value',
        'yet-another-test-header-override': 'yet-another-test-value',
  ...
  describe('fields', () => {
    ...
    it('should return empty fields if service config has no overrides', () => {
      getServiceConfigMock.mockImplementationOnce(() => mockServiceConfigWithoutOverrides)
