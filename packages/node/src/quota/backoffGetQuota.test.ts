...
describe('backoffGetQuota', () => {
  ...
  describe('events', () => {
    ...
    it('should emit quota enabled events when gradually re-enabling requests', async () => {
      ...
      expect(emitSpy).toHaveBeenLastCalledWith(events.internal.quota.enabled, {
        enabledPercent: 1,
      });
  ...
  describe('logging', () => {
    ...
    it('should not log info initially when enabled level is 100%', async () => {
    ...
    it('should stop logging on info level when re-enabled to 100%', async () => {
      randomSpy.mockReturnValue(0.99);
