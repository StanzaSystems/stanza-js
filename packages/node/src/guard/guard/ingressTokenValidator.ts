...
type TokenValidateResponse = CheckerResponse<'TOKEN_VALIDATE', unknown, { message: string }> & {
...
export const initIngressTokenValidator = (options: IngressTokenValidatorOptions) => {
  ...
  async function validateIngressToken (): Promise<TokenValidateResponse> {
    ...
    if (typeof (token) !== 'string' || token === '') {
      return validateFailure('TOKEN_NOT_VALID', 'Valid Stanza token was not provided in the incoming header')
    }
...
function validateFailure (tokenReason: TokenReason, message: string): TokenValidateResponse {
  return {
    type: 'TOKEN_VALIDATE',
    status: 'failure',
    reason: {
