...
const updateServiceConfigMock = vi.fn<
  Parameters<UpdateServiceConfig>,
  ReturnType<UpdateServiceConfig>
>();
const isServiceConfigInitializedMock = vi.fn<
  Parameters<IsServiceConfigInitialized>,
  ReturnType<IsServiceConfigInitialized>
>();
...
describe('fetchServiceConfig', () => {
  ...
  it('should call hub service with correct params', () => {
    ...
    expect(mockHubService.fetchServiceConfig).toHaveBeenCalledOnce();
    expect(mockHubService.fetchServiceConfig).toHaveBeenCalledWith({
      lastVersionSeen: '1',
      clientId: '2',
    });
  });
