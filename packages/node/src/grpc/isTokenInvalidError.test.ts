...
describe('isTokenInvalidError', () => {
  ...
  it.each([
    Object.assign(Error('boo'), {
      code: status.UNKNOWN,
      details:
        'failed to verify token: oidc: malformed jwt: square/go-jose: compact JWS format must have three parts',
      metadata: new Metadata()
    }),
    Object.assign(Error('boo'), {
      code: status.UNKNOWN,
      details:
        'failed to verify token: oidc: malformed jwt: illegal base64 data at input byte 344',
      metadata: new Metadata()
    }),
    Object.assign(Error('boo'), {
      code: status.UNKNOWN,
    ...
    Object.assign(Error('boo'), {
      ...
      details: 'request unauthenticated',
      metadata: new Metadata()
    })
