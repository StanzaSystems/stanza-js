...
const STANZA_GUARD_CONFIG_LISTENERS_SYMBOL = Symbol.for('Guard Config Listeners')
describe('guardConfig', function () {
  ...
  it('should stop notifying after listener unsubscribes', function () {
    ...
    const unsubscribe1 = guardConfigModule.addGuardConfigListener('testGuard', listener1)
    guardConfigModule.addGuardConfigListener('testGuard', listener2)
  ...
  it('should not fail when unsubscribing a listener multiple times', function () {
    ...
    const unsubscribe = guardConfigModule.addGuardConfigListener('testGuard', listener)
