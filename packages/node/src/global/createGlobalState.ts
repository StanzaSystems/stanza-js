...
const globalStatesRecord = createGlobal(Symbol.for('[Stanza SDK Internal] Global states'), (): Record<string | symbol, {
  changeListeners: Array<ChangeListener<any>>
  state: GlobalState<any>
}> => ({}))

export const createGlobalState = <S extends string | symbol, T>(s: S, createFn: () => T): GlobalState<T> => {
  ...
  const { changeListeners, state } = globalStatesRecord[s] = globalStatesRecord[s] ?? {
