...
export const addInstrumentation = async (serviceName: string) => {
  const { default: { HttpInstrumentation } } = await import('@opentelemetry/instrumentation-http')
  ...
  const { default: { NodeSDK } } = await import('@opentelemetry/sdk-node')
  const {
    default: {
      CompositePropagator, W3CTraceContextPropagator
    }
  } = await import('@opentelemetry/core')
  const { default: { Resource } } = await import('@opentelemetry/resources')
  const { default: { SemanticResourceAttributes } } = await import('@opentelemetry/semantic-conventions')
  const { default: { PeriodicExportingMetricReader } } = await import('@opentelemetry/sdk-metrics')
